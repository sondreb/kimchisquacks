  ...
  fetch("doctors-content.json")
    ...
    .then((data) => {
      ...
      data.forEach((doctor, index) => {
        ...
        cancerCountParagraph.innerHTML = `Number: ${
          index + 1
        }<br>Cancer Count: ${doctor.cancerCount}`;
        ...
        if (doctor.data && doctor.data.results) {
          doctor.data.results.forEach((result) => {
            if (result.addresses) {
              result.addresses.forEach((address) => {
                if (address.telephone_number) {
                  const phoneParagraph = document.createElement("p");
                  phoneParagraph.textContent = `Phone: ${address.telephone_number}`;
                  doctorDiv.appendChild(phoneParagraph);
                }
              });
            }
          });
        ...
        if (doctor.data) {
          ...
          dataParagraph.textContent = JSON.stringify(doctor.data);
